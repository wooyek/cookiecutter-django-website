...
class SampleDetailTests(UserTestCase):
    def test_anonymous(self):
        ...
        url = resolve_url("{{ cookiecutter.package_name }}:SampleDetail", item.pk)
...
class SampleCreateTests(UserTestCase):
    def test_anonymous(self):
        url = resolve_url('{{ cookiecutter.package_name }}:SampleCreate')
        response = test.Client().get(url)
        self.assertEqual(302, response.status_code)
    ...
    def test_post(self):
        ...
        self.assertRedirects(response, resolve_url('{{ cookiecutter.package_name }}:SampleDetail', item.pk), fetch_redirect_response=False)
